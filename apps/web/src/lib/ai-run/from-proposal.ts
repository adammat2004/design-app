...
function geometryChange(
  ...
  next: PlanGeometry,
): { kind: 'move' | 'resize' | 'rotate' | 'reshape' | 'reroute'; payload: Record<string, unknown> } | null {
  ...
  switch (next.kind) {
    case 'rect': {
      ...
      if (resized) {
        return { kind: 'resize', payload: { to: {
          centre: next.centre, width: next.width, depth: next.depth, rotation: next.rotation,
        } } };
      }
    ...
    case 'polygon': {
      ...
      if (samePoints(from.points, next.points) && from.cornerRadius === next.cornerRadius) return null;
      return { kind: 'reshape', payload: { to: { points: next.points, cornerRadius: next.cornerRadius } } };
    }
...
function propertyChanges(previous: DesignElement, next: DesignElement): Record<string, unknown> {
  const fields = [
    'name', 'category', 'material', 'edging', 'retaining', 'elevation', 'height',
    'symbol', 'plantId', 'plantingStyle', 'zone', 'hidden',
  ] as const;
...
export function proposedChangeToOperations(change: ProposedChange): LeafOperation[] {
  ...
  if (change.kind === 'add') {
    ...
    return [{ ...base, id: `${change.id}-add`, kind: 'add', ref: `$${change.id}`, element } as LeafOperation];
  }

  if (!change.elementId) return [];

  if (change.kind === 'remove')
    return [{ ...base, id: `${change.id}-remove`, kind: 'remove', elementId: change.elementId } as LeafOperation];
  ...
  for (const phase of PHASE_ORDER) {
    for (const leaf of leaves) {
      ...
      if (subject && subject !== selected) {
        operations.push({
          id: `${leaf.id}-select`, agent: leaf.agent, phase: leaf.phase,
          label: leaf.label, kind: 'select', elementId: subject,
        } as DesignOperation);
        selected = subject;
      }
